...
aldh.dat <- cell.dat[grepl('^ALDH',genes),]
```
...
Is there are subset of genes that, *a priori* would be preferred to differentiate among cell lineages?

```{r,eval=FALSE}
distances <- dist(t(aldh.dat), method="euclidean")
clusters <- hclust(distances,method="ward.D")
ggdendrogram(clusters,rotate=TRUE)
```

## 1) Commonly expressed genes

We have considered the expression pattern of some randomly selected genes.
...
for(i in random.samples) {
  #print(summary(cell.dat[i,]))
  tmp <- as.numeric(cell.dat[i,])
  hist(tmp,main=paste("Histogram, Row",i))
}
```
...
A final approach to dimension reduction is to use principal component analysis to indentify pseudovariables that capture variation in gene expression patterns. We present a PCA of the 1100 candidate genes identified in the previous step. PCA may be computionally intractable for the full data set.
