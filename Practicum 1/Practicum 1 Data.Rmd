...
### Pounds Gained
The skew of Pounds gained should be addressed. If this is our response variable, we would consider either data transformation, or a generalized linear model of the poisson family. 

Using `Pounds_gained` would result in 121 fewer responses.

The box-whisker plot `pounds_gained` vs `shift` shows a curvolinear response. We should be able to create a straightforward model predicting `weight_gained` from shift. The relationship between weight_gained and Calculated `Total Met Minutes` would require further exploratory analysis, as any relationship is not appearent from the scatterplot.
